...
여러 명의 회의 일정에서 사용자가 겪는 본질적 문제는 ‘빈 시간 찾기’가 아니라 ‘그 시간이 회의하기 진짜 괜찮은지 판단하는 일’이라고 생각했습니다. 누가 꼭 참석해야 하는지, 선택 참석자는 어디까지 고려할지, 점심 직후나 외근 이동, 앞뒤 일정 부담 같은 맥락은 보여주지 않습니다. 그래서 주최자는 빈 시간을 찾고도 메신저로 다시 확인하고, 참석자는 정해진 뒤에야 부담을 말하게 됩니다. 

즉, 캘린더의 빈 시간만으로는 참석자의 실제 가능 여부와 선호를 알기 어려워, 초대 후 다시 일정을 조율하는 합의 비용이 반복되는 것을 핵심 문제로 정의했습니다.
...
조율 요청 단계에서 참석자 응답 화면은 https://meet-fit-six.vercel.app/participant.html 에서, 자세한 설계 문서는 https://github.com/heexohee/MeetFit 에서 확인할 수 있습니다.
